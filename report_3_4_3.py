...
class Report:
    def __init__(self, user_input):
        self.heads_by_year = ['Год', f'Средняя зарплата - {user_input.job_name}', f'Количество вакансий - {user_input.job_name}']
    ...
    def generate_pdf(self, st, user_input):
        ...
        rows_1 = []
        for year, value in st.year_by_salary_job.items():
            rows_1.append([year, value, st.year_by_vac_num_job[year]])
        ...
        template = env.get_template(r"pdf_template_3_4_3.html")
        pdf_template = template.render({'job_name': user_input.job_name, 'img_path': img_path, 'table1': table1, 'table2': table2, 'area_name': user_input.area_name})
        config = pdfkit.configuration(wkhtmltopdf=r'C:\Program Files\wkhtmltopdf\bin\wkhtmltopdf.exe')
        pdfkit.from_string(pdf_template, 'report_3-4-3.pdf', configuration=config, options={"enable-local-file-access": ""})

    def generate_image(self, st, user_input):
        fig = plt.figure()
        width = 0.5
        x_n = np.arange(len(st.year_by_salary_job.keys()))
        x_list1 = x_n - width / 2
        x_list2 = x_n
        self.ax_1(st, user_input, fig, x_list2, x_n, width)
        self.ax_2(st, user_input, fig, x_list2, x_n, width)
        self.ax_3(st, fig)
        self.ax_4(st, fig)
    ...
    @staticmethod
    def ax_1(st, user_input, fig, x_list2, x_n, width):
        graf_1 = fig.add_subplot(221)
        graf_1.set_title(f'Уровень зарплат г.{user_input.area_name}')
        ...
        graf_1.set_xticks(x_n, st.year_by_salary_job.keys(), rotation='vertical')
        graf_1.tick_params(axis="both", labelsize=8)
        graf_1.legend(fontsize=8, loc='upper left')
        graf_1.grid(True, axis='y')

    @staticmethod
    def ax_2(st, user_input, fig, x_list2, x_n, width):
        graf_2 = fig.add_subplot(222)
        graf_2.set_title(f'Количество вакансий г.{user_input.area_name}')
        ...
        graf_2.set_xticks(x_n, st.year_by_vac_num_job.keys(), rotation='vertical')
